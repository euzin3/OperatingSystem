...
void simulate(process *procs, int proc_count) {
    ...
    // HW3 
    int active = 1; // 아직 남은 참조가 있는 프로세스가 있는지 여부
    while (active) {
        active = 0; // 일단 종료 상태로 간주하고, 남은 참조가 있으면 다시 1로 설정
        ...
        for (int i = 0; i < proc_count; i++) {
            ...
            active = 1; // 아직 참조할 게 있음

            unsigned char page = p->references[p->ref_count]; // 다음 참조할 페이지 번호

            pte *pt_entry = &p->page_table[page]; // 해당 페이지의 page table entry

            printf("[PID %02d IDX:%03d] %03d Page access: ", p->pid, p->ref_count, page);

            if (pt_entry->vflag == PAGE_VALID) {
                // 이미 매핑되어 있는 경우
                pt_entry->ref++;
                printf("Frame %03d\n", pt_entry->frame);
            } else {
                ...
                pt_entry->frame = new_frame;
                pt_entry->vflag = PAGE_VALID;
                pt_entry->ref = 1;
...
void print_page_tables(process *procs, int proc_count) {
    ...
    for (int i = 0; i < proc_count; i++) {
        ...
        printf("** Process %03d: Allocated Frames=%03d PageFaults/References=%03d/%03d\n",
               p->pid, p->page_faults, p->page_faults, p->ref_count);
        ...
        total_alloc_frames += p->page_faults + PAGETABLE_FRAMES;
    }
